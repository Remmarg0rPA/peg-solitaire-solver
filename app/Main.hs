...
flipPos pos board = [if pos /= p then (p, s) else (p, not s) | (p,s)<-board]
...
readBoard = concat . imap genPos
  where
    genPos i rw = [(V2 (fromIntegral x) (fromIntegral i), rw!!x=='X')
                  | x<-[0..length rw - 1], rw!!x/=' ']
...
bruteForceSolution board
  ...
  where
    ...
    allValidMoves = filter (`isValidMove` board) [(pos, dir)
                                                 | pos<-usedSqrs, dir<-directions]
    solution = filter (not . null . fst) [(bruteForceSolution (fromJust $ makeMove mv board), mv)
                                         | mv<-allValidMoves]
